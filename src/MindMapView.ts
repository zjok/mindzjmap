...
type LayoutNode = MindNodeData & { _sh?: number };
...
export class MindMapView extends TextFileView {
    ...
    async onOpen() {
        ...
        // padding override moved to styles.css
    ...
    private applyToolbarStyle() {
        ...
        const bdrW = s.toolbarBorderWidth ?? 1;
        // eslint-disable-next-line -- dynamic toolbar colors require inline style
    ...
    public applyStyle(p: Partial<MindMapStyle>) {
        ...
        if (
            ...
            p.toolbarBorderColor !== undefined ||
            p.toolbarBorderWidth !== undefined
        )
            this.applyToolbarStyle();
        if (
    ...
    private showStyleModal() {
        ...
        const R = (
            ...
            if (tp === "color") {
                ...
                i.addEventListener("change", () =>
                    this.applyStyle({ [k]: i.value } as Partial<MindMapStyle>),
                );
            } else if (tp === "number") {
                const i = row.createEl("input") as HTMLInputElement;
                i.type = "number";
                ...
                s.addEventListener("change", () =>
                    this.applyStyle({ [k]: s.value } as Partial<MindMapStyle>),
                );
            } else if (tp === "toggle") {
                const cb = row.createEl("input") as HTMLInputElement;
                cb.type = "checkbox";
                cb.checked = !!cv;
                cb.addClass("mz-modal-chk");
        ...
        tbRow
            .createEl("label", { text: t("sm.showToolbar") })
            .addClass("mz-modal-lbl");
    ...
    private showNodeStyleEditor(e: MouseEvent) {
        ...
        mkS(
            ...
            (v) =>
                apply((n) => {
                    n.styleOverride = n.styleOverride ?? {};
                    n.styleOverride.borderStyle = v as BorderStyle;
                }),
        );
        mkC(
            t("sm.borderColor"),
        ...
        if (isR) {
            ...
            const rebuildNL = () => {
                ...
                const mkNI = (
                    ...
                    it.addEventListener("mouseenter", () => {
                        if (key !== activePal) it.addClass("mz-rainbow-hover");
                    });
                    it.addEventListener("mouseleave", () => {
                        it.toggleClass("mz-rainbow-hover", false);
                    });
    ...
    private _kd = (e: KeyboardEvent) => {
        ...
        if (this.matchKey(e, this.kb.dragCanvas) && !e.repeat) {
            ...
            e.stopPropagation();
            this.spaceDown = true;
            if (this.cc) this.cc.toggleClass("mz-cursor-grab", true);
            return;
        }
    ...
    private cancelDrag() {
        ...
        this.ds = null;
        if (this.cc) this.cc.toggleClass("mz-cursor-grab", false);
        this.render();
    }
    ...
    private updTx() {
        // eslint-disable-next-line -- SVG group transform requires direct style for pan/zoom performance
    ...
    private cSH(n: MindNodeData): number {
        ...
        (n as LayoutNode)._sh = Math.max(n.height + vm, total);
        return (n as LayoutNode)._sh;
    }
    private cXY(n: MindNodeData) {
        ...
        if (rightCh.length) {
            let tot = 0;
            for (const c of rightCh) tot += (c as LayoutNode)._sh;
            let cy = n.y - tot / 2;
            for (const c of rightCh) {
                c.x = n.x + n.width / 2 + hm + c.width / 2;
                c.y = cy + (c as LayoutNode)._sh / 2;
                this.cXY(c);
                cy += (c as LayoutNode)._sh;
            }
        }
        // Layout left children (mirror)
        if (leftCh.length) {
            let tot = 0;
            for (const c of leftCh) tot += (c as LayoutNode)._sh;
            let cy = n.y - tot / 2;
    ...
    private startBox(lx: number, ly: number, shift: boolean) {
        ...
        // box-select pointer-events handled by CSS
    ...
    private _dt(
        ...
        // Block drop onto any selected node or its descendants
        const selIds = this.allSel();
        for (const sid of selIds) {
            if (n.id === sid || this.isDesc(n, sid)) return null;
        }
        if (n.id === did || this.isDesc(n, did)) return null;
        // Use right edge for left-side nodes, left edge for right-side nodes
        const x = leftX;
        const pad = 25;
    ...
    private childDF(lx: number, ly: number, dx: number, dy: number) {
        ...
        if (!d.ghostEl) {
            ...
            if (orig) orig.toggleClass("mz-drag-transparent", true);
            const dn = this.fAll(d.nodeId);
            if (dn) {
                ...
                // ghost opacity handled by mz-drag-ghost class
                // ghost pointer-events handled by mz-drag-ghost class
                ...
                // indicator pointer-events handled by CSS
