...
var DEFAULT_TOOLBAR_DARK = {
  toolbarBgColor: "#1e1e1e",
  toolbarBtnBgColor: "#2d2d2d",
  toolbarBtnBorderStyle: "solid",
  toolbarBtnBorderColor: "#4a4a4a",
  toolbarBtnTextColor: "#cccccc"
};
...
var MindMapView = class extends import_obsidian.TextFileView {
  constructor(leaf, plugin) {
    ...
    this._mu = (e) => {
      ...
      const gx = dn ? dn.x + dx : mx, gy = dn ? dn.y + dy : my;
  ...
  async onOpen() {
    ...
    ct.empty();
    ct.style.cssText = "display:flex;flex-direction:column;height:100%;padding-top:0!important;";
    const styleTag = document.createElement("style");
    styleTag.textContent = ".workspace-leaf-content .view-content{padding-top:0!important;}";
    ...
    this.mdCt.style.cssText = "width:100%;flex:1;padding:16px 24px;box-sizing:border-box;font-family:var(--font-monospace);display:none;font-size:14px;color:var(--text-normal);background:var(--background-primary);border:none;outline:none;resize:none;overflow:auto;white-space:pre;tab-size:4;order:1;";
    this.bindEvts();
  ...
  applyCanvasBg() {
    if (this.svgCt)
      this.svgCt.style.background = this.style.canvasBg;
  }
  applyToolbarStyle() {
    const s = this.style;
  ...
  updateToolbarVisibility() {
    if (this.tbEl)
      this.tbEl.style.display = this.plugin.settings.showToolbar ? "flex" : "none";
  }
  isSel(id) {
  ...
  updateDevPanel() {
    ...
    if (!this.devEl && this.svgCt) {
      ...
      this.devEl.style.cssText = "position:absolute;bottom:8px;left:8px;z-index:100;background:rgba(0,0,0,0.85);color:#0f0;font-family:monospace;font-size:11px;padding:8px 12px;border-radius:8px;max-width:400px;max-height:200px;overflow:auto;pointer-events:auto;white-space:pre;user-select:text;";
  ...
  buildTb(tb) {
    ...
    tb.createEl("div").style.cssText = "width:1px;height:20px;background:var(--background-modifier-border);";
    ...
    this.zoomLabel.style.cssText = "font-size:11px;color:var(--text-muted);min-width:40px;text-align:center;cursor:pointer;user-select:none;";
    ...
    h.style.cssText = "font-size:10px;color:var(--text-muted);margin-left:auto;white-space:nowrap;";
  ...
  applyStyle(p) {
    ...
    this.applyCanvasBg();
    if (p.toolbarPosition !== void 0 || p.toolbarPadTop !== void 0 || p.toolbarPadRight !== void 0 || p.toolbarPadBottom !== void 0 || p.toolbarPadLeft !== void 0 || p.toolbarBgColor !== void 0)
      this.applyToolbarStyle();
  ...
  showStyleModal() {
    ...
    m.style.cssText = "position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:var(--background-primary);border:1px solid var(--background-modifier-border);border-radius:14px;padding:24px;z-index:10000;min-width:440px;max-height:85vh;overflow-y:auto;box-shadow:0 12px 40px rgba(0,0,0,.25);";
    m.createEl("h3", { text: t("sm.title") }).style.cssText = "margin:0 0 16px;font-size:16px;";
    const close = () => {
      ov.remove();
      m.remove();
      this.popClose = null;
    ...
    const sec = (icon, title) => {
      ...
      s.createEl("div", { text: icon + " " + title }).style.cssText = "font-weight:700;color:var(--text-accent);font-size:13px;margin-bottom:10px;padding-bottom:6px;border-bottom:2px solid var(--background-modifier-border);";
      return s;
    };
    const R = (p, l, k, tp, o) => {
      var _a, _b, _c;
      const row = p.createEl("div");
      row.style.cssText = "display:flex;align-items:center;justify-content:space-between;margin-bottom:7px;gap:10px;";
      row.createEl("label", { text: l }).style.cssText = "font-size:12px;flex:1;color:var(--text-normal);";
      const cv = this.style[k];
      if (tp === "color") {
        const i = row.createEl("input");
        i.type = "color";
        i.value = typeof cv === "string" && cv.startsWith("var") ? "#ffffff" : String(cv);
        i.style.cssText = "width:44px;height:26px;border:none;cursor:pointer;border-radius:4px;";
        i.addEventListener(
          "change",
        ...
        i.value = String(cv);
        i.style.cssText = "width:64px;padding:4px;border:1px solid var(--background-modifier-border);border-radius:6px;font-size:12px;background:var(--background-secondary);color:var(--text-normal);";
        i.addEventListener(
          "change",
          () => this.applyStyle({ [k]: Number(i.value) })
        );
      } else if (tp === "select") {
        const s = row.createEl("select");
        s.style.cssText = "padding:4px;border:1px solid var(--background-modifier-border);border-radius:6px;font-size:12px;background:var(--background-secondary);color:var(--text-normal);";
        ...
        cb2.type = "checkbox";
        cb2.checked = !!cv;
        cb2.style.cssText = "width:18px;height:18px;cursor:pointer;";
        cb2.addEventListener(
          "change",
          () => this.applyStyle({ [k]: cb2.checked })
        );
      }
    };
    ...
    }).style.cssText = "font-size:12px;color:var(--text-normal);display:block;margin-bottom:6px;";
    const allPals = {
      ...RAINBOW_PALETTES,
    ...
    listWrap.style.cssText = "max-height:200px;overflow-y:auto;border:1px solid var(--background-modifier-border);border-radius:8px;";
    const rebuildList = () => {
      listWrap.innerHTML = "";
      const updated = {
        ...RAINBOW_PALETTES,
      ...
      const mkItem = (key, label, colors) => {
        ...
        it.addEventListener("mouseleave", () => {
          it.style.background = key === curPal ? "var(--background-modifier-hover)" : "transparent";
        });
        const nm = it.createEl("span", { text: label });
        nm.style.cssText = "min-width:60px;font-weight:" + (key === curPal ? "700" : "400") + ";flex-shrink:0;";
        if (colors) {
          const dots = it.createEl("span");
          dots.style.cssText = "display:flex;gap:1px;flex:1;flex-wrap:wrap;";
        ...
        if (key !== "off" && !RAINBOW_PALETTES[key]) {
          ...
          del.style.cssText = "margin-left:auto;color:var(--text-muted);cursor:pointer;font-size:11px;padding:2px 4px;flex-shrink:0;";
    ...
    addBtn.style.cssText = "padding:4px 12px;border:1px dashed var(--background-modifier-border);background:var(--background-secondary);border-radius:6px;cursor:pointer;font-size:11px;";
    addBtn.addEventListener("click", () => {
      ...
      nameInp.style.cssText = "width:120px;padding:3px 6px;border:1px solid var(--background-modifier-border);border-radius:4px;font-size:11px;margin-bottom:6px;display:block;";
      const colors = [
        "#ff0000",
        "#00ff00",
        "#0000ff",
      ...
      cDiv.style.cssText = "display:flex;gap:3px;flex-wrap:wrap;align-items:center;margin-bottom:6px;";
      const rebuild = () => {
        cDiv.innerHTML = "";
        for (let i = 0; i < colors.length; i++) {
          ...
          ci.type = "color";
          ci.value = colors[i];
          ci.style.cssText = "width:24px;height:24px;border:none;cursor:pointer;border-radius:4px;";
          ci.addEventListener("change", () => {
            colors[i] = ci.value;
          });
        }
        const ab = cDiv.createEl("button", { text: "+" });
        ab.style.cssText = "width:24px;height:24px;border:1px dashed var(--text-muted);background:none;border-radius:50%;cursor:pointer;font-size:14px;color:var(--text-muted);";
      ...
      saveBtn.style.cssText = "padding:4px 12px;border:none;background:var(--interactive-accent);color:var(--text-on-accent);border-radius:6px;cursor:pointer;font-size:12px;";
    ...
    tbRow.style.cssText = "display:flex;align-items:center;justify-content:space-between;margin-bottom:7px;";
    tbRow.createEl("label", { text: t("sm.showToolbar") }).style.cssText = "font-size:12px;flex:1;";
    ...
    tbResetBtn.style.cssText = "margin-top:6px;width:100%;padding:5px;border:1px solid var(--background-modifier-border);background:var(--background-secondary);border-radius:6px;cursor:pointer;font-size:11px;";
    ...
    br.style.cssText = "display:flex;gap:8px;justify-content:flex-end;margin-top:16px;";
    ...
    cb.style.cssText = "padding:6px 16px;border:none;background:var(--interactive-accent);color:var(--text-on-accent);border-radius:8px;cursor:pointer;";
  ...
  showNodeStyleEditor(e) {
    ...
    const ov = document.createElement("div");
    ov.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,.4);z-index:9999;";
    const m = document.createElement("div");
    m.style.cssText = "position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:var(--background-primary);border:1px solid var(--background-modifier-border);border-radius:14px;padding:20px;z-index:10000;min-width:360px;max-height:75vh;overflow-y:auto;box-shadow:0 12px 40px rgba(0,0,0,.25);";
    m.createEl("h4", {
    ...
    const mkC = (l, cur, fn) => {
      ...
      r.style.cssText = "display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;gap:10px;";
      r.createEl("span", { text: l }).style.cssText = "font-size:12px;flex:1;";
      const i = r.createEl("input");
      i.type = "color";
      i.value = cur;
      i.style.cssText = "width:40px;height:24px;border:none;cursor:pointer;border-radius:4px;";
    ...
    const mkN = (l, cur, min, max, fn) => {
      ...
      i.max = String(max);
      i.value = String(cur);
      i.style.cssText = "width:60px;padding:3px;border:1px solid var(--background-modifier-border);border-radius:6px;font-size:12px;";
    ...
    const mkS = (l, cur, opts, fn) => {
      ...
      s.style.cssText = "padding:3px;border:1px solid var(--background-modifier-border);border-radius:6px;font-size:12px;";
      for (const o of opts) {
        const op = document.createElement("option");
        op.value = o;
        op.text = o;
    ...
    if (isR) {
      ...
      ctDiv.createEl("span", {
        text: "\u{1F308} " + t("sm.rainbow")
      }).style.cssText = "font-size:12px;display:block;margin-bottom:6px;";
      const nPals = {
        ...RAINBOW_PALETTES,
        ...this.style.customRainbows || {}
      };
      ...
      nList.style.cssText = "max-height:160px;overflow-y:auto;border:1px solid var(--background-modifier-border);border-radius:6px;";
      const rebuildNL = () => {
        nList.innerHTML = "";
        const mkNI = (key, label, colors) => {
          ...
          it.addEventListener("mouseleave", () => {
            it.style.background = key === activePal ? "var(--background-modifier-hover)" : "transparent";
          });
          it.createEl("span", { text: label }).style.cssText = "min-width:60px;font-weight:" + (key === activePal ? "700" : "400") + ";flex-shrink:0;";
          if (colors) {
            const dots = it.createEl("span");
    ...
    clr.style.cssText = "margin-top:10px;width:100%;padding:6px;border:1px solid var(--background-modifier-border);background:var(--background-secondary);border-radius:8px;cursor:pointer;font-size:12px;";
    ...
    cbtn.style.cssText = "margin-top:8px;width:100%;padding:6px;border:none;background:var(--interactive-accent);color:var(--text-on-accent);border-radius:8px;cursor:pointer;";
  ...
  toggleSearch() {
    ...
    this.searchBar.style.cssText = "position:absolute;top:10px;right:10px;z-index:100;display:flex;gap:4px;align-items:center;background:var(--background-primary);border:1px solid var(--background-modifier-border);border-radius:10px;padding:6px 12px;box-shadow:0 4px 16px rgba(0,0,0,.15);";
    const inp = document.createElement("input");
    inp.type = "text";
    inp.placeholder = t("search.placeholder");
    inp.style.cssText = "width:180px;padding:5px 10px;border:1px solid var(--background-modifier-border);border-radius:6px;font-size:13px;background:var(--background-secondary);color:var(--text-normal);outline:none;";
    const info = document.createElement("span");
    info.style.cssText = "font-size:11px;color:var(--text-muted);min-width:40px;text-align:center;";
    const closeBtn = document.createElement("button");
    closeBtn.innerText = "\u2715";
    closeBtn.style.cssText = "border:none;background:none;cursor:pointer;font-size:14px;color:var(--text-muted);padding:2px 4px;";
  ...
  calcW(txt, isR) {
    ...
    for (const l of txt.split("\n")) {
      ...
      d.style.cssText = "font-family:" + ff + ";font-size:" + fs + "px;font-weight:bold;visibility:hidden;position:absolute;white-space:pre;";
      d.innerText = l || " ";
  ...
  childDF(lx, ly, dx, dy) {
    ...
    if (!d.ghostEl) {
      ...
      if (dn2) {
        ...
        gd.style.cssText = "width:100%;height:100%;display:flex;align-items:center;justify-content:center;border-radius:" + this.style.nodeBorderRadius + "px;font-weight:bold;font-size:14px;background:var(--background-secondary);color:var(--text-normal);outline:2px dashed var(--interactive-accent);";
...
var StylePanelView = class extends import_obsidian2.ItemView {
  ...
  build() {
    ...
    ct.createEl("h4", { text: t("sm.title") }).style.cssText = "margin:0 0 14px;";
    const s = ((_a = this.getView()) == null ? void 0 : _a.style) ? { ...this.getView().style } : this.plugin.getStyle();
    const upd = (p) => {
    ...
    const mkSec = (icon, title) => {
      ...
      sec.style.cssText = "margin-bottom:18px;border:1px solid var(--background-modifier-border);border-radius:10px;overflow:hidden;";
      sec.createEl("div", { text: icon + "  " + title }).style.cssText = "padding:8px 12px;background:var(--background-secondary);font-weight:700;font-size:12px;color:var(--text-accent);border-bottom:1px solid var(--background-modifier-border);";
      const b = sec.createEl("div");
      b.style.cssText = "padding:10px 12px;display:flex;flex-direction:column;gap:7px;";
      return b;
    };
    const mkRow = (p, l) => {
      const r = p.createEl("div");
      r.style.cssText = "display:flex;align-items:center;justify-content:space-between;gap:8px;";
      r.createEl("span", { text: l }).style.cssText = "flex:1;font-size:12px;";
      return r;
    };
    const mkColor = (p, l, k) => {
      const r = mkRow(p, l);
      ...
      i.type = "color";
      const v = s[k];
      i.value = typeof v === "string" && v.startsWith("var") ? "#ffffff" : String(v);
      i.style.cssText = "width:36px;height:22px;border:none;cursor:pointer;border-radius:4px;";
      i.addEventListener("input", () => upd({ [k]: i.value }));
    };
    const mkRange = (p, l, k, min, max, step) => {
      ...
      n.style.cssText = "width:30px;text-align:right;color:var(--text-muted);font-size:11px;";
    ...
    const mkNum = (p, l, k, min, max) => {
      ...
      i.max = String(max);
      i.value = String(s[k]);
      i.style.cssText = "width:60px;padding:2px 4px;border:1px solid var(--background-modifier-border);border-radius:4px;font-size:11px;";
      i.addEventListener(
        "change",
        () => upd({ [k]: Number(i.value) })
      );
    ...
    const mkSel = (p, l, k, opts) => {
      ...
      sel.style.cssText = "padding:2px 4px;border:1px solid var(--background-modifier-border);border-radius:4px;font-size:11px;";
      for (const o of opts) {
        const op = document.createElement("option");
        op.value = o;
        op.text = o;
    ...
    const mkTog = (p, l, k) => {
      ...
      cb.type = "checkbox";
      cb.checked = !!s[k];
      cb.style.cssText = "width:16px;height:16px;cursor:pointer;accent-color:var(--interactive-accent);";
      cb.addEventListener(
        "change",
        () => upd({ [k]: cb.checked })
      );
    ...
    lw.style.cssText = "max-height:160px;overflow-y:auto;border:1px solid var(--background-modifier-border);border-radius:6px;";
    const mkI = (key, label, colors) => {
      ...
      it.addEventListener("mouseleave", () => {
        it.style.background = key === curP ? "var(--background-modifier-hover)" : "transparent";
      });
      it.createEl("span", { text: label }).style.cssText = "min-width:55px;font-weight:" + (key === curP ? "700" : "400") + ";";
      if (colors) {
    ...
    br.createEl("button", { text: t("sm.reset") }).addEventListener(
      ...
      () => {
        ...
        if (v)
          ...
          this.plugin.updateStyle({ ...DEFAULT_STYLE });
        this.build();
      }
    );
    const rf = br.createEl("button", { text: "\u21BB" });
    rf.style.cssText = "flex:1;padding:6px;border:none;background:var(--interactive-accent);color:var(--text-on-accent);border-radius:8px;cursor:pointer;font-size:12px;";
...
var MindMapSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    about.style.cssText = "text-align:center;padding:24px 16px;margin-bottom:20px;background:var(--background-secondary);border-radius:12px;border:1px solid var(--background-modifier-border);";
    about.createEl("div").innerText = "\u{1F5FA}\uFE0F";
    about.lastElementChild.setAttribute(
      "style",
      "font-size:48px;margin-bottom:8px;"
    );
    about.createEl("h2", { text: "MindZJ" }).style.cssText = "margin:0 0 4px;font-size:22px;";
    about.createEl("div", {
    ...
    about.createEl("p", { text: t("set.desc") }).style.cssText = "font-size:13px;margin:0 0 8px;line-height:1.5;max-width:400px;margin-left:auto;margin-right:auto;";
    const ghLink = about.createEl("div").createEl("a", {
      text: `\u{1F4E6} ${t("set.github")}`,
      href: "https://github.com/zjok/mindzj"
    });
    ghLink.style.cssText = "color:var(--text-accent);text-decoration:none;font-size:13px;";
    ghLink.setAttr("target", "_blank");
    const donSec = containerEl.createEl("div");
    donSec.style.cssText = "padding:16px;margin-bottom:20px;background:var(--background-secondary);border-radius:12px;border:1px solid var(--background-modifier-border);";
    donSec.createEl("h3", { text: `\u2615 ${t("set.donate")}` }).style.cssText = "margin:0 0 12px;font-size:15px;text-align:center;";
    const donBtns = donSec.createEl("div");
    donBtns.style.cssText = "display:flex;gap:10px;justify-content:center;flex-wrap:wrap;";
    ...
    const mkNameSetting = (name, desc, field, defVal) => {
      ...
      resetEl.style.cssText = "border:none;background:none;cursor:pointer;font-size:16px;color:var(--text-muted);padding:2px 6px;display:none;";
      ...
      inp.style.cssText = "width:140px;padding:4px 8px;border:1px solid var(--background-modifier-border);border-radius:6px;font-size:13px;";
    ...
    for (const item of keyMap) {
      ...
      const resetEl = setting.controlEl.createEl("button", { text: "\u21BA" });
      resetEl.style.cssText = "border:none;background:none;cursor:pointer;font-size:16px;color:var(--text-muted);padding:2px 6px;display:" + (curVal === defVal ? "none" : "inline") + ";";
      resetEl.addEventListener("click", async () => {
        kb[item.key] = defVal;
        await this.plugin.saveSettings();
        this.plugin.refreshAllViews();
      ...
      setting.addButton((b) => {
        ...
        b.onClick(() => {
          ...
          const handler = (e) => {
            ...
            resetEl.style.display = combo === defVal ? "none" : "inline";
          };
...
var MindNodePlugin = class extends import_obsidian4.Plugin {
    ...
    this.registerEvent(
      this.app.workspace.on("file-menu", (menu, file) => {
        if (file instanceof import_obsidian4.TFolder)
          menu.addItem(
            (i) => i.setTitle("New MindZJ").setIcon("network").onClick(() => this.createInFolder(file))
          );
        const items = menu.items;
        if ((items == null ? void 0 : items.length) > 1) {
          const mzj = items.pop();
          const idx = items.findIndex(
            (it) => {
              var _a;
              return (((_a = it.titleEl) == null ? void 0 : _a.textContent) || it.title) === "New folder";
            }
          );
          if (idx >= 0) {
            mzj.section = items[idx].section;
  ...
  async loadSettings() {
    ...
    if (d) {
      this.settings = {
        ...
        toolbarStyleLight: (_j = d.toolbarStyleLight) != null ? _j : void 0,
        toolbarStyleDark: (_k = d.toolbarStyleDark) != null ? _k : void 0
      };
      const valid = [
        "bezier",
        "straight",
  ...
  async switchTheme(newTheme) {
    ...
    if (newTheme === "light") {
      this.applyToolbarColors(
        (_a = this.settings.toolbarStyleLight) != null ? _a : DEFAULT_TOOLBAR_LIGHT
      );
    } else {
      this.applyToolbarColors(
        (_b = this.settings.toolbarStyleDark) != null ? _b : DEFAULT_TOOLBAR_DARK
      );
    }
    await this.saveSettings();
  ...
  getRootName() {
    ...
    return (cn == null ? void 0 : cn.root) || t("def.root");
  }
  getChildName() {
    const cn = this.settings.customNodeNames[this.settings.language];
    return (cn == null ? void 0 : cn.child) || t("def.child");
  }
  refreshAllViews() {
